...
// Only application APIs belong on the public Vercel front door. Operational
// pages such as /servers, /cluster, /update and the manager dashboard must be
// reached on a main server and must never be forwarded by this catch-all.
const PUBLIC_API_PREFIXES = new Set(["v1", "v2", "v3", "v4"]);
...
async function proxyRequest(request, pathSegments = []) {
  ...
  if (!PUBLIC_API_PREFIXES.has(firstSegment)) {
    return Response.json(
      { error: "not found" },
      {
        status: 404,
  ...
  let target;
  try {
    target = await getProxyTarget();
  } catch (err) {
    console.error(`[front-door] target lookup failed: ${err.message}`);
    return Response.json(
