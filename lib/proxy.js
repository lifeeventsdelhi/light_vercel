...
// Keep the server registry and cluster details off the public Vercel front
// door. Every other path, including the manager home page and application
// APIs, is forwarded to the active main.
const PRIVATE_MANAGER_PREFIXES = new Set(["servers", "cluster"]);
...
async function proxyRequest(request, pathSegments = []) {
  ...
  if (PRIVATE_MANAGER_PREFIXES.has(firstSegment)) {
    return Response.json(
      { error: "not found" },
      {
        status: 404,
