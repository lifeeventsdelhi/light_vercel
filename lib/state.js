...
async function evaluate(pingerRole) {
  ...
  const activeIsType1 = Boolean(type1 && state.active_main === type1.server_id);
  const activeIsType2 = Boolean(type2 && state.active_main === type2.server_id);
  const activeIsKnownMain = activeIsType1 || activeIsType2;

  // The schema's initial seed uses the conventional "main-t1" id, while
  // deployments may register another id. Correct an unknown seed/state on
  // the first real main ping rather than proxying through a stale identifier.
  if (!activeIsKnownMain && pingerRole === "main_server_type_1" && type1) {
    await performSwitch(type1, "corrected active_main from type_1 registry ping");
  ...
  if (
    ...
    await performSwitch(
      type2,
      `type_1 ping stale > ${config.STALE_SWITCH_MIN} min (age=${
        !type1 || type1.ping_age_min === null ? "never" : Number(type1.ping_age_min).toFixed(1)
      })`
    );
...
async function getProxyTarget() {
  ...
  if (!target) {
    const rows = await sql`
      SELECT s.server_id, s.cloudflare_tunnel_address
        FROM servers s
        LEFT JOIN server_pings p ON p.server_id = s.server_id
       WHERE s.role IN ('main_server_type_1', 'main_server_type_2')
         AND s.status = 'online'
         AND COALESCE(s.cloudflare_tunnel_address, '') <> ''
       ORDER BY p.pinged_at DESC NULLS LAST, s.role
       LIMIT 1`;
    if (rows[0]) {
      target = {
        activeMain: rows[0].server_id,
